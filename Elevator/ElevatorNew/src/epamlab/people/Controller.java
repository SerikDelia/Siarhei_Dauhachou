...
public class Controller {
	...
	public void movingElevator() {
		...
		if (upDirection) {
		    if (hasElevatorUp()) {
				if (isElevatorEmpty() && isFloorEmpty() && elevator.getCountGoingOut()!=ConstantElevator.passengersNumber) {			
					elevatorUp();
					logger
				...
				if( elevator.getCountGoingOut()!=ConstantElevator.passengersNumber ){								
				elevatorUp();
	...
	public void waitUpdatePainLiftUp() {
		synchronized (paintLiftWaitUP) {
			...
			int   currentYL=liftY-200;
			if(ConstantElevator.animationBoost>=10){
			while (liftY!=currentYL) {
				...
				while (liftY!=currentYL) {
                   ...
                   while (liftY!=currentYL) {
						...
						liftY=liftY-ConstantElevator.animationBoost;

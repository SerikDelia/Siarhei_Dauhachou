...
public class MainWindows extends JFrame implements Runnable {
	...
	public MainWindows(List<Floor> floors, Controller controller,
		...
		if((((ConstantElevator.storiesNumber*200)))>800){
			this.setSize(1024,800);
		setLocation((int)((size.getWidth()-1024)/2), 
				((int)(size.getHeight()-800)/2));
		}else{		
			this.setSize(1024,(ConstantElevator.storiesNumber*200));
			setLocation((int)((size.getWidth()-1024)/2), 
					((int)(size.getHeight()-(ConstantElevator.storiesNumber*200))/2));
		}
		...
		scroll.setPreferredSize(new Dimension(1024,(ConstantElevator.storiesNumber*200)));
